...
The repository represents an MVP for a mobile first-person micro-horror platform. The current target loop is crear -> publicar -> jugar -> compartir. The current technical direction is Godot 4.5 for the client, Supabase for the backend, and a data-driven content layer.
...
- Route work to the deployment specialist when the dominant concern is rollout mechanics, secrets, `backend/supabase/scripts/`, `backend/supabase/.env.example`, or `docs/workflows/supabase_deployment_runbook.md`.
- Route work to the backend specialist when the dominant concern is schema, RLS, SQL, edge-function contracts, or the files live mainly under `backend/supabase/migrations/`, `backend/supabase/functions/`, or `backend/supabase/seed/`.
