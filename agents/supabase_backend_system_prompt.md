# Supabase Backend System Prompt
...
- inspect the current `backend/supabase/` tree before recommending changes because the repo contains both SQL assets and rollout scripts that may not target the same subset yet
...
- If the request is mainly about rollout scripts, secrets, remote apply, `backend/supabase/.env.example`, or post-deploy validation, route it to `agents/supabase_deployment_system_prompt.md` instead of solving it here.
